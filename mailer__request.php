...
try {
    //Server settings
    $mail->SMTPDebug = 0; // Set debug output to off
    $mail->isSMTP();                                     //Send using SMTP
    $mail->Host       = 'smtp.gmail.com';                //Set the SMTP server to send through
    $mail->SMTPAuth   = true;                            //Enable SMTP authentication
    $mail->Username   = 'bckyrd.io@gmail.com';           //SMTP username
    $mail->Password   = 'egvp ldqp ufwj zvjg';           //SMTP password
    ...
    if (isset($_POST['submit_approve'])) {
        ...
        $mail->Subject = 'Real Estate Request For Visit ';
        $mail->Body    = $notificationMessage;
        $mail->AltBody = $notificationMessage;
        $mail->send();

        // After executing the database update and other operations
        echo "<script>alert('Schedule email Sent.'); window.location.href='property__admin__request.php';</script>";
    }
} catch (Exception $e) {
